"""
Tomorrow Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.keshav/ (configs, sessions, logs)
"""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Tomorrow" comment and following line
                if '# Tomorrow' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``tomorrow`` command:
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``tomorrow gateway run`` processes
...
#   4. On the ``tomorrow dashboard`` + gateway paths, drops files into
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``tomorrow -p <name> gateway stop|uninstall`` because
    ...
    #    Use `python -m hermes_cli.main` so we don't depend on a `tomorrow`
...
def run_gui_uninstall(args):
    ...
    Mirrors ``tomorrow uninstall --gui``. Removes the desktop app's built
    ...
    print("or 'tomorrow uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.keshav/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.keshav/ for future reinstall
    """
    ...
    # full wipe (code + ~/.keshav data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Tomorrow Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
...
def _perform_uninstall(
    *,
    ...
    ``tomorrow`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.keshav/, just remove the hermes-agent subdir
    ...
    # 5. Optionally remove ~/.keshav/ data directory (and named profiles)
    if full_uninstall:
    ...
    print()
    print("Thank you for using Tomorrow! ⚕")
    print()
