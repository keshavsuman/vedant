...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or vedant -c.",
    ...
    "Ctrl+Z suspends Vedant to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "vedant -c resumes your most recent CLI session. vedant -c \"project name\" resumes by title.",
    "vedant -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "vedant -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "vedant chat -t web,terminal enables only specific toolsets for a focused session.",
    "vedant chat -s github-pr-workflow preloads a skill at launch.",
    "vedant chat -q \"query\" runs a single non-interactive query and exits.",
    "vedant chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "vedant chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "vedant --yolo bypasses all dangerous command approval prompts for the entire session.",
    "vedant chat --source telegram tags the session for filtering in vedant sessions list.",
    "vedant -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "vedant doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "vedant dump outputs a compact setup summary — great for bug reports.",
    "vedant config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "vedant config edit opens config.yaml in your default editor.",
    "vedant config check scans for missing or stale configuration options.",
    "vedant sessions browse opens an interactive session picker with search.",
    "vedant sessions stats shows session counts by platform and database size.",
    "vedant sessions prune --older-than 30 cleans up old sessions.",
    "vedant skills search react --source skills-sh searches the skills.sh public directory.",
    "vedant skills check scans installed hub skills for upstream updates.",
    "vedant skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "vedant skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "vedant mcp add github --command npx adds MCP servers from the command line.",
    "vedant mcp serve runs Vedant itself as an MCP server for other agents.",
    "vedant auth add lets you add multiple API keys for credential pool rotation.",
    "vedant completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "vedant logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "vedant backup creates a zip backup of your entire Vedant home directory.",
    "vedant profile create coder creates an isolated profile that becomes its own command.",
    "vedant profile create work --clone copies your current config and keys to a new profile.",
    "vedant update syncs new bundled skills to ALL profiles automatically.",
    "vedant gateway install sets up Vedant as a system service (systemd/launchd).",
    "vedant memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "vedant webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: vedant tools disables unused tools, vedant skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "vedant models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as vedant -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Vedant tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'vedant profile create coder' creates the 'coder' command.",
    "vedant profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Vedant prints a resume command with session ID and stats.",
    "vedant sessions export backup.jsonl exports all sessions for backup or analysis.",
    "vedant -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "vedant skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .vedant/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: vedant cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Vedant runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "vedant gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Vedant via a local macOS server.",
    ...
    "Context files (.vedant.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Vedant loads project context from .vedant.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "vedant mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "vedant mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "vedant mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — vedant tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.localstreet/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "vedant chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "vedant plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "vedant acp runs Vedant as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "vedant auth add nous or vedant auth add openai-codex sets up OAuth-based providers.",
    ...
    "vedant status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.localstreet/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Vedant your own.",
    ...
    "Before a gateway session reset, Vedant auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.localstreet/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.localstreet/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'vedant profile create ls' would be rejected.",
    "vedant profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "vedant claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via vedant tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.localstreet/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.localstreet/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Vedant auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Vedant config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Vedant is working.',
    ...
    '/reload reloads ~/.localstreet/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.localstreet/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.localstreet/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'vedant curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "vedant curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'vedant curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'vedant auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'vedant dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'vedant dashboard embeds the full Vedant TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.localstreet/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.localstreet/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.localstreet/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.localstreet/skills/ so drop-in skills appear without restarting the session.',
    ...
    'vedant -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'vedant chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'vedant chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'vedant chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "vedant chat --source tool tags programmatic chats so they don't clutter vedant sessions list.",
    'vedant dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'vedant sessions rename <ID> "new title" renames any past session; vedant sessions delete <ID> removes one.',
    'vedant import restores a full Vedant backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'vedant fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'vedant pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'vedant setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'vedant status --deep runs the full health sweep across every component; plain vedant status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Vedant block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from vedant sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.localstreet/dashboard-plugins/.',
]
