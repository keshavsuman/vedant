...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or tomorrow -c.",
    ...
    "Ctrl+Z suspends Tomorrow to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "tomorrow -c resumes your most recent CLI session. tomorrow -c \"project name\" resumes by title.",
    "tomorrow -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "tomorrow -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "tomorrow chat -t web,terminal enables only specific toolsets for a focused session.",
    "tomorrow chat -s github-pr-workflow preloads a skill at launch.",
    "tomorrow chat -q \"query\" runs a single non-interactive query and exits.",
    "tomorrow chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "tomorrow chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "tomorrow --yolo bypasses all dangerous command approval prompts for the entire session.",
    "tomorrow chat --source telegram tags the session for filtering in tomorrow sessions list.",
    "tomorrow -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "tomorrow doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "tomorrow dump outputs a compact setup summary — great for bug reports.",
    "tomorrow config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "tomorrow config edit opens config.yaml in your default editor.",
    "tomorrow config check scans for missing or stale configuration options.",
    "tomorrow sessions browse opens an interactive session picker with search.",
    "tomorrow sessions stats shows session counts by platform and database size.",
    "tomorrow sessions prune --older-than 30 cleans up old sessions.",
    "tomorrow skills search react --source skills-sh searches the skills.sh public directory.",
    "tomorrow skills check scans installed hub skills for upstream updates.",
    "tomorrow skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "tomorrow skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "tomorrow mcp add github --command npx adds MCP servers from the command line.",
    "tomorrow mcp serve runs Tomorrow itself as an MCP server for other agents.",
    "tomorrow auth add lets you add multiple API keys for credential pool rotation.",
    "tomorrow completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "tomorrow logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "tomorrow backup creates a zip backup of your entire Tomorrow home directory.",
    "tomorrow profile create coder creates an isolated profile that becomes its own command.",
    "tomorrow profile create work --clone copies your current config and keys to a new profile.",
    "tomorrow update syncs new bundled skills to ALL profiles automatically.",
    "tomorrow gateway install sets up Tomorrow as a system service (systemd/launchd).",
    "tomorrow memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "tomorrow webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: tomorrow tools disables unused tools, tomorrow skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "tomorrow models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as tomorrow -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Tomorrow tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'tomorrow profile create coder' creates the 'coder' command.",
    "tomorrow profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Tomorrow prints a resume command with session ID and stats.",
    "tomorrow sessions export backup.jsonl exports all sessions for backup or analysis.",
    "tomorrow -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "tomorrow skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .tomorrow/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: tomorrow cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Tomorrow runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "tomorrow gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Tomorrow via a local macOS server.",
    ...
    "Context files (.tomorrow.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Tomorrow loads project context from .tomorrow.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "tomorrow mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "tomorrow mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "tomorrow mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — tomorrow tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.keshav/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "tomorrow chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "tomorrow plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "tomorrow acp runs Tomorrow as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "tomorrow auth add nous or tomorrow auth add openai-codex sets up OAuth-based providers.",
    ...
    "tomorrow status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.keshav/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Tomorrow your own.",
    ...
    "Before a gateway session reset, Tomorrow auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.keshav/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.keshav/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'tomorrow profile create ls' would be rejected.",
    "tomorrow profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "tomorrow claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via tomorrow tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.keshav/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.keshav/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Tomorrow auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Tomorrow config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Tomorrow is working.',
    ...
    '/reload reloads ~/.keshav/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.keshav/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.keshav/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'tomorrow curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "tomorrow curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'tomorrow curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'tomorrow auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'tomorrow dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'tomorrow dashboard embeds the full Tomorrow TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.keshav/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.keshav/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.keshav/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.keshav/skills/ so drop-in skills appear without restarting the session.',
    ...
    'tomorrow -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'tomorrow chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'tomorrow chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'tomorrow chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "tomorrow chat --source tool tags programmatic chats so they don't clutter tomorrow sessions list.",
    'tomorrow dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'tomorrow sessions rename <ID> "new title" renames any past session; tomorrow sessions delete <ID> removes one.',
    'tomorrow import restores a full Tomorrow backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'tomorrow fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'tomorrow pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'tomorrow setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'tomorrow status --deep runs the full health sweep across every component; plain tomorrow status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Tomorrow block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from tomorrow sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.keshav/dashboard-plugins/.',
]
