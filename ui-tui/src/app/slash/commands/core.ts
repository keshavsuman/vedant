...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Tomorrow to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `tomorrow update`.
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Tomorrow #${i + 1}`
