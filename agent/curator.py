...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.localstreet/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.localstreet/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.localstreet/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.localstreet/logs/curator/``)
    ...
    authored skill data in ``~/.localstreet/skills/``.
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.localstreet/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.localstreet/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.localstreet/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.localstreet/skills/.archive/` and are recoverable by `mv`")
