...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.keshav/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.keshav/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.keshav/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.keshav/logs/curator/``)
    ...
    authored skill data in ``~/.keshav/skills/``.
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.keshav/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.keshav/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.keshav/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.keshav/skills/.archive/` and are recoverable by `mv`")
